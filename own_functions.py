    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            if test_prediction == 1:
                ...
                xboxes.append(((xbox_left, ytop_draw+ystart),(xbox_left+win_draw,ytop_draw+win_draw+ystart)))

    return xboxes, draw_img
...
# Define a single function that can extract features using hog sub-sampling and make predictions
def find_cars_opt(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins):
    cspace = 'YCrCb'
    #draw_img = np.copy(img)
   # img = img.astype(np.float32)/255
    if cspace != 'RGB':
